...
/** User-facing explanation for a plan-blocked Apollo call. Apollo's Free plan returns 403
 * API_INACCESSIBLE for the People Search / People Enrichment endpoints this app relies on
 * (confirmed against the live key's own API profile), regardless of how the account's email is
 * registered. Shared between the thrown error, the enrich routes' 409 body, and the README so
 * the wording never drifts. */
export const APOLLO_PLAN_BLOCK_MESSAGE =
  "Apollo's Free plan does not include the people search and enrichment API. Upgrade to a paid Apollo plan, then save a new API key in Settings.";
