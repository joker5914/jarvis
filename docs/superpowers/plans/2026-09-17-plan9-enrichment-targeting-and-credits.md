...
**Fix round (review of commit `68b076f`, corrections to the text above):**

- **P1/B2** — the PATCH `markAsChain` line above (originally: "appends `normalizeName(name)`... to `overrides.exclusion.chains`") is corrected: it uses a new `chainKeyFor(name)` helper (`src/lib/scoring/smbFit.ts`), not `normalizeName`. `normalizeName` strips `&`/`'`/`-`/`.`, so "H&R Block" would normalize to "h r block" — a string `hasWord` (which `scoreSmbFit` matches against the raw, punctuation-intact lowercase name) could never match again. `chainKeyFor` lowercases, trims, collapses whitespace, and strips only a trailing legal-entity suffix (`llc|inc|co|corp|ltd|pllc|pc`), keeping every other character intact.
- **P2/B1** — the same line's "appends ... to `overrides.exclusion.chains`" is corrected to **"appends ... to the *effective* chain list (`cfg.exclusion.chains` — defaults merged with any prior override)"**, then persists that whole list back as the override. `mergeConfig` REPLACES `chains` wholesale when an override is present (it does not merge element-wise — see `runtime.ts`), so seeding from the raw override alone would have persisted just the one new entry on the first click and silently dropped every one of the default seed chains for that owner from then on.
- **P3** — the `rescoreExclusions` line above ("Businesses whose current reasons include `apollo_headcount:` keep that reason... merge, do not overwrite") is corrected/generalized: the function only ever calls `scoreSmbFit` with `{ name, sameNameCount }` (no `estimatedCost`/`workType`), so it can only ever regenerate a `chain:`, `entity:`, or `same_name_count:` reason — never `cost_over_*` or `public_right_of_way`. Every reason this pass has no information to regenerate (a TDLR cost/right-of-way reason, or `chain:apollo_headcount:*`) is preserved unchanged, whatever `scoreSmbFit` concludes about the name today; only the reasons it *can* recompute are replaced by its fresh output. A business stays excluded as long as at least one reason (recomputed or preserved) remains.
- **P5** — the PATCH response is `{ business, rescore }` where `rescore` is the *full* `{ scanned, newlyExcluded, restored }` summary (not just `newlyExcluded` as the original line said). For `markAsChain`, `newlyExcluded` counts only businesses whose new `exclusionReasons` include the specific chain key just added (via a before/after snapshot), not every business `rescoreExclusions` happened to flip that pass for an unrelated reason.
- **Un-mark path (new, not in the original Task 3 text):** PATCH also accepts `clearChain: z.literal(true).optional()` (mutually exclusive with `markAsChain`) — "Restore as SMB" in `LeadDetail.tsx`, shown in place of "Not an SMB (chain)" once a business is excluded for a `chain:`-prefixed reason. It removes `chainKeyFor(name)` from the effective chain list (if present) and strips `chain:`-prefixed reasons (both name-derived and `apollo_headcount`) from *this business only* — deliberately scoped, not a table-wide `rescoreExclusions` call, since undoing a chain-list entry could otherwise silently restore an unreviewed number of other businesses. Response: `rescore: { scanned: 1, newlyExcluded: 0, restored: 1|0 }`.
- **Seed list correction (N7/N8):** `"geico", "planet fitness", "great clips", "supercuts", "jiffy lube", "aspen dental"` were removed — same franchise/agent-owned rule as `"snap fitness"`/`"state farm"`. `"spectrum", "goodyear", "firestone", "staples", "best buy"` were also removed as common-word/surname/city false positives.
- **Wording correction (N1):** the headcount guard's activity message and `chainHeadcountMin`'s doc comment both said "people"/"headcount"; corrected to "decision-makers" everywhere (`isEnrichIssueMessage`'s regex too) — every People Search call is filtered by `preferredTitles`/`seniorities`, so Apollo's `total_entries` here was never a raw employee count. The persisted `chain:apollo_headcount:<n>` reason string is unchanged (it's a matched-on contract, not user-facing copy).
- `scripts/chain-sweep.ts` does **not** call `rescoreExclusions` on `--apply` (only `markAsChain` does) — it marks just the specific businesses the sweep found over the threshold, not their look-alikes.
